...
def test_house_book_oos_supports_explicit_date_cutoff(tmp_path: Path) -> None:
    closed_csv = tmp_path / "closed.csv"
    closed_csv.write_text(
        "\n".join(
            [
                "house_position_id,opened_at,closed_at,entry_notional_usdc,exit_cost_total_usdc,realized_pnl_net_usdc",
                "p1,2025-12-15T00:00:00+00:00,2025-12-20T00:00:00+00:00,100,110,10",
                "p2,2026-01-10T00:00:00+00:00,2026-01-15T00:00:00+00:00,120,130,-3",
            ]
        ),
        encoding="utf-8",
    )
    ...
    open_csv.write_text(
        "\n".join(
            [
                "house_position_id,opened_at,entry_notional_usdc,mtm_pnl_net_usdc,mark_price,analysis_cutoff",
                "p3,2026-02-01T00:00:00+00:00,90,4,0.60,2026-03-30T00:00:00+00:00",
            ]
        ),
        encoding="utf-8",
    )
    ...
    summary_rows = {row["split"]: row for row in results["summary_rows"]}
    assert summary_rows["train_before_2026-01-01"]["n_positions"] == 1
    assert summary_rows["test_from_2026-01-01"]["n_positions"] == 2
